...
describe("charity navlink", function () {
  ...
  it("checks if the admin can add charity", function () {
    //cy.refreshDatabase();
    ...
    cy.getById("login_form").submit(); // cy.wait(10000);
    // should go to dashbaord page"

    cy.url().should("eq", "http://localhost:3000/dashboard"); // cy.assertRedirect('/dashboard');
    //cy.get(':nth-child(3) > .ant-menu-submenu-title > .ant-menu-title-content').click()

    cy.visit("http://localhost:3000/charity");
    cy.get("a > .ant-btn > span").click(); //cy.contains("[data-cy='addStaff']", "Add Staff").click();
  ...
  it("checks if the admin can not  add with empty field charity", function () {
    ...
    cy.get("#name").type("gergesoihon");
    cy.get("#description").type("it is charity org work on mental ill people");
    cy.get("#category").type("62824778d6233ba729718764"); // cy.get("#email").type("kebdele@gmail.com");

    cy.get("#address").type("Addis Ababa");
    cy.get("#phone").type("093465843");
    cy.get(".ant-form-item-control-input-content > .ant-btn > span").click();
  });
  it("checks if the admin can edit charity", function () {
    cy.getById("logout_button").click(); // cy.wait(3000);
  ...
  it('checks if the admin can delete charity', function () {
    cy.visit("http://localhost:3000/login");
    cy.getById("username").type("Adem", {
      delay: 100
    }).should("have.value", "Adem");
    ...
    cy.get('.ant-modal-footer > .ant-btn-primary > span').click(); // cy.refreshDatabase();
  });
});